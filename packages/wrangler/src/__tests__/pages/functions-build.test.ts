...
describe("functions build", () => {
	...
	it("should throw an error if no [--build-output-directory] was specified and the `/functions` directory was not found, when called with [--experimental-worker-bundle=true]", async () => {
		await expect(
			runWrangler("pages functions bundle --experimental-worker-bundle=true")
		).rejects.toThrowErrorMatchingInlineSnapshot(`
		"Could not find a static assets directory or the Functions directory.
		No [--directory] of static files was provided so we looked for /functions but couldn't find anything.
			➤ If you are trying to build _worker.js, please make sure you provide the [--directory] containing your static files.
...
export default {
  async fetch(request, env) {
	...
	it("should build _worker.js over /functions, if both are present, a valid [--build-output-directory] is provided, and the [--experimental-worker-bundle] flag is set", async () => {
		/* ---------------------------- */
		/*       Set up _worker.js      */
		/* ---------------------------- */
		mkdirSync("public");
...
export default {
  async fetch(request, env) {
    ...
    `
		...
		/* --------------------------------- */
		await runWrangler(
			`pages functions build --experimental-worker-bundle=true`
		);
